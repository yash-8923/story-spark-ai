...
const ForgotPasswordComponent = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // TODO: wire up to backend forgot-password API endpoint
    setSubmitted(true);
  };
        ...
        {submitted ? (
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-gray-500">
              ...
              <span className="font-medium text-gray-700">{email}</span>, we've sent a password reset link.
            </p>
            <Link to="/login" className="inline-block mt-4 text-sm font-medium text-indigo-600 hover:text-indigo-500">
              &larr; Back to login
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            >
              Send reset link
            </button>
